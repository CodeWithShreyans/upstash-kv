import kv, { UpstashKV, createClient } from ".";
...
jest.mock("@upstash/redis", () => ({
    Redis: jest.fn(() => ({
        ...
        scan: jest
            .fn()
            .mockImplementation(() =>
                Promise.resolve(scanReturnValues.shift())
            ),
        // eslint-disable-next-line jest/unbound-method
        scanIterator: UpstashKV.prototype.scanIterator,
    })),
}));
...
describe("upstash-kv", () => {
    ...
    describe("kv export", () => {
        it("exports a default client", async () => {
            ...
            process.env.UPSTASH_REST_API_TOKEN = "tok_foobar";

            expect(await kv.get("foo")).toEqual("bar");

            process.env.UPSTASH_REST_API_URL = undefined;
            process.env.UPSTASH_REST_API_TOKEN = undefined;
        });
    });
    ...
    describe("scanIterator", () => {
        it("terminates iteration for trivial case", async () => {
            const client = new UpstashKV({ url: "foobar", token: "foobar" });
